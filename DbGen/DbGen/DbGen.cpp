...
static void genAsyncCbInterface(const OutputFile& out, fstream& fs, const Elem& e, const DataTypeArray& types) {
  al(fs, "package "+out.getPackage()+";");
  AnsiString name;
  if(e.isSelect()) {
    Select s = e.asSelect();
    al(fs, "public interface " + toUpper(s.getName()) + "Cb {");
    al(fs, "    public void onResult("+ typeMapper(s.getType()) + " result);");
  } else if(e.isInsert()) {
    Insert i = e.asInsert();
    al(fs, "public interface " + toUpper(i.getName()) + "Cb {");
    al(fs, "    public void onResult();");
  }
  al(fs, "    public void onError();");
  al(fs, "};");
}

/*
static void genAsyncSelect(fstream& fs, const Select& s, const DataTypeArray& types, const InputFile& f) {
  genAsyncCbInterface(fs, Elem::createSelect(s), types, f);
  genAsyncRunnable(fs, s, types, f);
  const SelectParams& params = s.getParams();
  al(fs, "  static public void " + s.getName() + "(DBConn db" + mkParams(params) + ", " + toUpper(s.getName())  + "Cb cb) {");
  AnsiString runParams;
  for (int i = 0; i < params.Size(); i++)
    runParams += ", " + params[i].getName();
  al(fs, "    " + toUpper(s.getName()) + "Runnable thread = new " + toUpper(s.getName()) + "Runnable(db " + runParams + ", cb);");
  al(fs, "    thread.run();");
  al(fs, "  }");
}*/
...
//TODO
/*
static void generateAsyncFile(const OutputFile& out, const InputFile& in, const DataTypeArray& types) {
  std::fstream fs;
  doOpen(fs,  out.getName() + ".java");
  al(fs, "package " + out.getPackage() + ";");
  al(fs, "import data.*;");
  ...
  for (int i = 0; i < elems.Size(); i++) {
    Elem e = elems[i];
    if (e.isRecord())
      throw Exception("Move type declarations to *.dd InputFile");
    else if (e.isArray())
      throw Exception("Move type declarations to *.dd InputFile");
    else if (e.isSelect())
      genAsyncSelect(fs, e.asSelect(), types, out, in);
    else if (e.isInsert())
      genAsyncInsert(fs, e.asInsert(), types, out, in);
    else {
      //todo insert/update
      throw Exception("update not supported yet");
    }
  }
  al(fs, "};");
}
...
static void genAsyncApiResponseCb(const OutputFile& out, fstream& fs, const Elem& e, const DataTypeArray& types) {
  if(e.isSelect()) {
    const Select& s = e.asSelect();
    const SelectParams& params = s.getParams();
    al(fs, "package "+out.getPackage()+";");
    al(fs, "import mednt.mhtml.MAjaxCallback;");
    al(fs, "public class " + toUpper(s.getName()) + "ApiResponseCb implements MAjaxCallback {");
    al(fs, "    private " + toUpper(s.getName()) + "Cb cb;");
    al(fs, "    public " + toUpper(s.getName()) + "ApiResponseCb(" + toUpper(s.getName()) + "Cb cb) {");
    al(fs, "      this.cb = cb;");
    al(fs, "    }");
    al(fs, "    public void onResult(String response) {");
    al(fs, "      try {");
    al(fs, "         ApiResponse responseKind = ApiResponse.fromJSON(response);");
    al(fs, "         if (responseKind.isResult()) {");
    al(fs, "             "+ typeMapper(s.getType()) +" result = "+ typeMapper(s.getType()) +".fromJSON(responseKind.asResult());");
    al(fs, "             this.cb.onResult(result);");
    al(fs, "         } else { ");
    al(fs, "             this.cb.onError();");
    al(fs, "         }");
    al(fs, "      } catch(Exception e) {");
    al(fs, "        this.cb.onError();");
    al(fs, "      }");
    al(fs, "    }");
    al(fs, "    public void onError() {");
    ...
    const Insert& i = e.asInsert();
    const SelectParams& params = i.getParams();
    al(fs, "package "+out.getPackage()+";");
    al(fs, "import mednt.mhtml.MAjaxCallback;");
    al(fs, "public class " + toUpper(i.getName()) + "ApiResponseCb implements MAjaxCallback {");
    al(fs, "    private " + toUpper(i.getName()) + "Cb cb;");
    al(fs, "    public " + toUpper(i.getName()) + "ApiResponseCb(" + toUpper(i.getName()) + "Cb cb) {");
    al(fs, "      this.cb = cb;");
    al(fs, "    }");
    al(fs, "    public void onResult(String response) {");
    al(fs, "      try {");
...
static void genJ2JApiClientSelect(fstream& fs, const Select& s, const DataTypeArray& types, const OutputFile& out) {
  const SelectParams& params = s.getParams();
  const AnsiString& path = out.getKind().asJ2jApiClient();
  al(fs, "  public static void " + s.getName() + "(MService conn" + mkParams(params) + ", " + toUpper(s.getName()) + "Cb cb) {");
  al(fs, "    Params params = new Params();");
  for (int i = 0; i < params.Size(); i++) {
    if (params[i].getType().isPlain()) {
      al(fs, "    params.Insert(Param.create" + toUpper(typeMapper(params[i].getType())) + "Param(" + params[i].getName() + "));");
    }
    else {
      al(fs, "    params.Insert(Param.create" + toUpper(typeMapper(params[i].getType())).SubString(1, typeMapper(params[i].getType()).Length()-2) + "AParam(" + out.getName() + ".convCodegenArray(" + params[i].getName() + ")));");
    }
  }
  al(fs, "    ApiQuery apiQuery = ApiQuery.createSelect(new SelectQuery(\"" + s.getName() + "\", params));");
  al(fs, "    try {");
  al(fs, "      conn.request(\"GET\", \""+path+"?api=\"+apiQuery.toJSON(), \"\", new " + toUpper(s.getName()) + "ApiResponseCb(cb));");
  al(fs, "    } catch(Exception e) {");
  al(fs, "      cb.onError();");
  al(fs, "    }");
  al(fs, "  }");
...
static void genJ2JApiClientInsert(fstream& fs, const Insert& s, const DataTypeArray& types, const OutputFile& out) {
  const SelectParams& params = s.getParams();
  const AnsiString& path = out.getKind().asJ2jApiClient();
  al(fs, "  public static void " + s.getName() + "(MService conn" + mkParams(params) + ", " + toUpper(s.getName()) + "Cb cb) {");
  al(fs, "    Params params = new Params();");
...
static void genJ2JApiClientUpdate(fstream& fs, const Update& s, const DataTypeArray& types, const OutputFile& out) {
  const SelectParams& params = s.getParams();
  const AnsiString& path = out.getKind().asJ2jApiClient();
  al(fs, "  public static void " + s.getName() + "(MService conn" + mkParams(params) + ", " + toUpper(s.getName()) + "Cb cb) {");
  al(fs, "    Params params = new Params();");
...
static void generateJ2JApiClient(const DbFile& dbFile, const OutputFile& out, const DataTypeArray& types) {
  const InputFile& in = dbFile.getInputFile();
  std::fstream fs;
  doOpen(fs,  out.getName() + ".java");
  al(fs, "package " + out.getPackage() + ";");
  ...
  al(fs, "import java.util.Map;");
  al(fs, "import mednt.mhtml.MAjaxCallback;");
  al(fs, "import mednt.mhtml.MService;");
  al(fs, "public class " + out.getName() + " {");
  al(fs, "  public static IntArray convCodegenArray(int[] array) {");
  al(fs, "    IntArray ret = new IntArray();");
  al(fs, "    for(int i=0;i<array.length;i++)");
  al(fs, "      ret.Insert(array[i]);");
  al(fs, "    return ret;");
  al(fs, "  }");
  al(fs, "  public static BoolArray convCodegenArray(boolean[] array) {");
  al(fs, "    BoolArray ret = new BoolArray();");
  al(fs, "    for(int i=0;i<array.length;i++)");
  al(fs, "      ret.Insert(array[i]);");
  al(fs, "    return ret;");
  al(fs, "  }");
  al(fs, "  public static StringArray convCodegenArray(String[] array) {");
  al(fs, "    StringArray ret = new StringArray();");
  al(fs, "    for(int i=0;i<array.length;i++)");
  al(fs, "      ret.Insert(array[i]);");
  al(fs, "    return ret;");
  al(fs, "  }");
  ...
  for (int i = 0; i < elems.Size(); i++) {
    Elem e = elems[i];
    if (e.isSelect())
      genJ2JApiClientSelect(fs, e.asSelect(), types, out);
    else if (e.isInsert())
      genJ2JApiClientInsert(fs, e.asInsert(), types, out);
    else if (e.isUpdate()) {
      genJ2JApiClientUpdate(fs, e.asUpdate(), types, out);
    }
  }
  al(fs, "};");
  for (int i = 0; i < elems.Size(); i++) {
    Elem e = elems[i];
    if (e.isSelect() || e.isInsert()) {
      AnsiString name = e.isSelect() ? e.asSelect().getName() : e.asInsert().getName();
      std::fstream fs;
      doOpen(fs, toUpper(name) + "Cb.java");
      genAsyncCbInterface(out, fs, e, types);
      fs.close();
      doOpen(fs, toUpper(name) + "ApiResponseCb.java");
      genAsyncApiResponseCb(out, fs, e, types);
      fs.close();
    }
  }
}
static void generateJavaApiServlet(const DbFile& dbFile, const OutputFile& out, const AnsiString& savePath) {
  std::fstream fs;
  doOpen(fs, savePath);
  al(fs, "package " + out.getPackage() +";");;
  al(fs, "import java.io.IOException;");
  al(fs, "import java.io.PrintWriter;");
  al(fs, "import java.nio.charset.StandardCharsets;");
  al(fs, "import javax.servlet.ServletException;");
  al(fs, "import javax.servlet.annotation.*;");
  al(fs, "import javax.servlet.http.*;");
  al(fs, "import mednt.dbconn.DBConn;");
  al(fs, "import mednt.dbconn.SDBResult;");
  al(fs, "import mednt.lib.Err;");
  al(fs, "@WebServlet(\"/MainServlet\")");
  al(fs, "public class MainServlet extends HttpServlet {");
  al(fs, "  public MainServlet() {");
  al(fs, "    super();");
  al(fs, "  }");
  al(fs, "  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{");
  al(fs, "    DBConn db = null;");
  al(fs, "    try {");
  al(fs, "      String basePath = getServletContext().getInitParameter(\"basePath\");");
  al(fs, "      String jdbcString = getServletContext().getInitParameter(\"jdbcString\");");
  al(fs, "      String jdbcUser = getServletContext().getInitParameter(\"jdbcUser\");");
  al(fs, "      String jdbcPasswd = getServletContext().getInitParameter(\"jdbcPasswd\");");
  al(fs, "      db = new DBConn(jdbcString, jdbcUser, jdbcPasswd, false);");
  al(fs, "      System.out.println(request.getParameter(\"api\"));");
  al(fs, "      String content = \"\";");
  al(fs, "      try {");
  al(fs, "        String rjson = " + getJavaProcessorOutputFile(dbFile).getName() +".process(db, request.getParameter(\"api\"));");
  al(fs, "        content = ApiResponse.createResult(rjson).toJSON();");
  al(fs, "      } catch (Exception e) {");
  al(fs, "        content = ApiResponse.createError(e.toString()).toJSON();");
  al(fs, "      }");
  al(fs, "      response.setContentType(\"text/html\");");
  al(fs, "      response.setCharacterEncoding(\"UTF-8\");");
  al(fs, "      response.setStatus(HttpServletResponse.SC_OK);");
  al(fs, "      response.getWriter().write(content);");
  al(fs, "      response.getWriter().flush();");
  al(fs, "      response.getWriter().close();");
  al(fs, "    } catch (Exception e) {");
  al(fs, "      throw Err.err(e);");
  al(fs, "    }");
  al(fs, "    if (db != null)");
  al(fs, "     db.close();");
  al(fs, "  }");
  al(fs, "  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{");
  al(fs, "    doGet(request, response);");
  al(fs, "  }");
  al(fs, "};");
}
...
static void generateJavaApiServer(const DbFile& dbFile, const OutputFile& out, const DataTypeArray& types) {
  ...
  generateJavaApiServlet(dbFile, out, "MainServlet.java");
}
...
int main(int argc, char* argv[]) {
  ...
  for(int i=0;i<f.getOutputFiles().Size();i++) {
    ...
    if(f.getOutputFiles()[i].getKind().isJavaAsync()) {
      //generateSyncFile(f, f.getOutputFiles()[i], dfb.getTypes());
    }
    if(f.getOutputFiles()[i].getKind().isJavaApiProcessor()) {
      generateJavaApiServer(f, f.getOutputFiles()[i], dfb.getTypes());
    }
    if(f.getOutputFiles()[i].getKind().isJ2jApiClient()) {
      generateJ2JApiClient(f, f.getOutputFiles()[i], dfb.getTypes());
    }
  }
}
